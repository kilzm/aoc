...
parse_input :: proc(input: string) -> (seeds: []i64, mappings: [NMAPS]Map) {
    ...
    for i := 3; i < len(lines); i += 1 {
        ...
        append(&mappings[mi], MapRule{start = start, end = start + nums[2], offset = nums[0] - start})
...
@(private ="file")
get_min_location_p2 :: proc(seed_ranges: []SeedRange, mappings: [NMAPS]Map) -> (min_location: i64) {
    min_location = bits.I64_MAX
...
map_ranges :: proc(seed_ranges: []SeedRange, mapping: Map) -> []SeedRange {
    map_single_range :: proc(range: SeedRange, mapping: Map) -> []SeedRange {
        ...
        for rule in mapping {
            ...
            if rstart < rend {
                append(&new_seed_ranges, SeedRange{
                    start = rstart + rule.offset,
                    end = rend + rule.offset,
                })
